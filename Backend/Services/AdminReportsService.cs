...
public class AdminReportsService : IAdminReportsService
{
    ...
    public async Task<ApiResponse<RevenueReportDto>> GetRevenueAsync(DateTime? from, DateTime? to, string groupBy)
    {
    ...
    public async Task<ApiResponse<List<TopProductDto>>> GetTopProductsAsync(DateTime? from, DateTime? to, int take)
    {
        var (start, end) = NormalizeRange(from, to);
        ...
        return ApiResponse<List<TopProductDto>>.Ok(products);
    }

    public async Task<ApiResponse<List<LowStockDto>>> GetLowStockAsync(int? threshold, int take)
    {
        ...
        return ApiResponse<List<LowStockDto>>.Ok(items);
    }

    public async Task<byte[]> ExportRevenueAsync(DateTime? from, DateTime? to, string groupBy)
    {
        var (start, end) = NormalizeRange(from, to);
        var report = await BuildRevenueReport(start, end, groupBy);
